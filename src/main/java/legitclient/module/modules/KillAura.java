...
import legitclient.events.*;
import legitclient.management.RotationState;
import legitclient.mixin.IAccessorPlayerControllerMP;
...
public class KillAura extends Module {
    ...
    private boolean performAttack(float yaw, float pitch) {
        if (!LegitClient.playerStateManager.digging && !LegitClient.playerStateManager.placing) {
    ...
    private boolean canAttack() {
        if (this.inventoryCheck.getValue() && mc.currentScreen instanceof GuiContainer) {
            ...
            if (((IAccessorPlayerControllerMP) mc.playerController).getIsHittingBlock()) {
                ...
                AutoHeal autoHeal = (AutoHeal) LegitClient.moduleManager.modules.get(AutoHeal.class);
                if (autoHeal.isEnabled() && autoHeal.isSwitching()) {
                    return false;
                } else {
                    BedNuker bedNuker = (BedNuker) LegitClient.moduleManager.modules.get(BedNuker.class);
                    if (bedNuker.isEnabled() && bedNuker.isReady()) {
                        return false;
                    } else if (LegitClient.moduleManager.modules.get(Scaffold.class).isEnabled()) {
                        return false;
    ...
    public void onUpdate(UpdateEvent event) {
        if (event.getType() == EventType.POST && this.blinkReset) {
            ...
            LegitClient.blinkManager.setBlinkState(false, BlinkModules.AUTO_BLOCK);
            LegitClient.blinkManager.setBlinkState(true, BlinkModules.AUTO_BLOCK);
        }
                    ...
                    switch (this.autoBlock.getValue()) {
                        case 0:
                            if (PlayerUtil.isUsingItem()) {
                                ...
                                if (!this.isPlayerBlocking() && !LegitClient.playerStateManager.digging && !LegitClient.playerStateManager.placing) {
                                    swap = true;
                                }
                            } else {
                                this.isBlocking = false;
                        ...
                        case 2:
                            if (this.hasValidTarget()) {
                                ...
                                if (LegitClient.playerStateManager.digging
                                        || LegitClient.playerStateManager.placing
                        ...
                        case 5:
                            if (this.hasValidTarget()) {
                                int item = ((IAccessorPlayerControllerMP) mc.playerController).getCurrentPlayerItem();
                                if (mc.thePlayer.inventory.currentItem == item && !LegitClient.playerStateManager.digging && !LegitClient.playerStateManager.placing) {
                                    switch (this.blockTick) {
                                        case 0:
                                            if (!this.isPlayerBlocking()) {
                                                swap = true;
                        ...
                        case 8:
                            ...
                            if (PlayerUtil.isUsingItem()
                                    ...
                                    && !LegitClient.playerStateManager.digging
                                    && !LegitClient.playerStateManager.placing) {
                                swap = true;
                            }
                    }
                ...
                if (this.isBoxInSwingRange(this.target.getBox())) {
                    if (this.rotations.getValue() == 2 || this.rotations.getValue() == 3) {
                        ...
                        if (this.rotations.getValue() == 3) {
                            LegitClient.rotationManager.setRotation(rotations[0], rotations[1], 1, true);
                        }
            ...
            if (this.showTarget.getValue() != 0
                ...
                switch (this.showTarget.getValue()) {
                    case 1:
                        ...
                        break;
                    case 2:
                        color = ((HUD) LegitClient.moduleManager.modules.get(HUD.class)).getColor(System.currentTimeMillis());
                }
