...
import legitclient.mixin.IAccessorGuiChat;
import legitclient.module.Module;
import legitclient.util.ColorUtil;
...
public class HUD extends Module {
    ...
    public void onTick(TickEvent event) {
        if (this.isEnabled() && event.getType() == EventType.POST) {
            this.activeModules = LegitClient.moduleManager.modules.values().stream().filter(module -> module.isEnabled() && !module.isHidden()).sorted(Comparator.comparingInt(this::getModuleWidth).reversed()).collect(Collectors.<Module>toList());
        }
    }
    ...
    public void onRender2D(Render2DEvent event) {
        if (this.chatOutline.getValue() && mc.currentScreen instanceof GuiChat) {
            ...
            if (LegitClient.commandManager != null && LegitClient.commandManager.isTypingCommand(text)) {
        ...
        if (this.isEnabled() && !mc.gameSettings.showDebugInfo) {
            ...
            if (this.blinkTimer.getValue()) {
                BlinkModules blinkingModule = LegitClient.blinkManager.getBlinkingModule();
                if (blinkingModule != BlinkModules.NONE && blinkingModule != BlinkModules.AUTO_BLOCK) {
                    long movementPacketSize = LegitClient.blinkManager.countMovement();
