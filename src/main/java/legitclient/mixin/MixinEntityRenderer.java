...
import legitclient.module.modules.*;
...
public abstract class MixinEntityRenderer {
    ...
    private void updateCameraAndRender(float float1, long long2, CallbackInfo callbackInfo) {
        if (this.mc.thePlayer != null) {
            ...
            KillAura killAura = (KillAura) LegitClient.moduleManager.modules.get(KillAura.class);
    ...
    private float hurtCameraEffect(float float1) {
        if (LegitClient.moduleManager == null) {
            ...
            NoHurtCam noHurtCam = (NoHurtCam) LegitClient.moduleManager.modules.get(NoHurtCam.class);
    ...
    private void a(
        ...
        if (LegitClient.moduleManager != null) {
            GhostHand event = (GhostHand) LegitClient.moduleManager.modules.get(GhostHand.class);
            if (event.isEnabled()) {
    ...
    private double v(Vec3 vec31, Vec3 vec32) {
        if (LegitClient.moduleManager == null) {
            ...
            return LegitClient.moduleManager.modules.get(ViewClip.class).isEnabled() ? (double) this.thirdPersonDistance : vec31.distanceTo(vec32);
        }
    }
    ...
    private Material x(Block block) {
        if (LegitClient.moduleManager == null) {
            ...
            return LegitClient.moduleManager.modules.get(ViewClip.class).isEnabled() ? Material.air : block.getMaterial();
        }
    }
    ...
    private boolean y(EntityLivingBase entityLivingBase, Potion potion) {
        if (potion == Potion.blindness && LegitClient.moduleManager != null) {
            AntiDebuff antiDebuff = (AntiDebuff) LegitClient.moduleManager.modules.get(AntiDebuff.class);
    ...
    private boolean c(EntityPlayerSP entityPlayerSP, Potion potion) {
        if (potion == Potion.confusion && LegitClient.moduleManager != null) {
            AntiDebuff antiDebuff = (AntiDebuff) LegitClient.moduleManager.modules.get(AntiDebuff.class);
