...
public class CommandManager {
    ...
    public void handleCommand(String string) {
        ...
        if (params.get(0).isEmpty()) {
            ChatUtil.sendFormatted(String.format("%sUnknown command&r", LegitClient.clientName).replace("&", "§"));
        } else {
            ...
            ChatUtil.sendFormatted(String.format("%sUnknown command (&o%s&r)&r", LegitClient.clientName, params.get(0)).replace("&", "§"));
        }
    }
